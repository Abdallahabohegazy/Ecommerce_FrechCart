  ...
  return (
    <>
      <div className='my-4 container'>
        <h3 className='mb-3 text-center text-md-start'>Shop Popular Categories</h3>
        <Slider  {...settings}>
          {data?.data.data.map((categorie) => (
            <div className="px-1 px-md-2" key={categorie._id}>
              <img src={categorie.image} height={200} className='w-100 d-block rounded' alt={categorie.name} />
              <h6 className='text-center mt-2 small'>{categorie.name}</h6>
            </div>
          ))}
        </Slider>
      </div>
